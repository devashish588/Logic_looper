...
function AppContent() {
  ...
  useEffect(() => {
    ...
    dispatch(resetGame());
    dispatch(
      loadSettings({ theme: 'dark', timerEnabled: true, soundEnabled: true }),
    );
    ...
    (async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (serverData) {
            dispatch(loadStats(serverData));
            statsLoaded = true;
            // Prepare for IndexedDB hydration
            if (serverData.heatmapData) {
              fetchedScores = Object.entries(serverData.heatmapData).map(
                ([date, score]) => ({
                  userId,
                  date,
                  score,
                  synced: true,
                  solved: true,
                  // we don't have full details here, but the core 'solved' status is what matters for the UI
                }),
              );
            }
          }
      ...
      // Hydrate IndexedDB if we got data from backend
      if (statsLoaded && fetchedScores.length > 0) {
        const { saveDailyActivity } =
          await import('./storage/activityStore.js');
        for (const score of fetchedScores) {
          await saveDailyActivity(userId, score);
        }
      }

      // 2. Fall back to user-scoped IndexedDB if backend failed
      if (!statsLoaded) {
        try {
  ...
  async function handleLogout() {
    // 1. Final sync guard: push any unsynced data before clearing
    if (userId && isAuthenticated) {
      const token = store.getState().auth.token;
      try {
        await syncDailyActivity(userId, token, { force: true });
      } catch (e) {
        console.warn('Final logout sync failed:', e);
      }
    }

    // 2. Reset ALL Redux state to defaults — clean slate for next user
    dispatch(resetStats());
    dispatch(resetGame());
    dispatch(
      loadSettings({ theme: 'dark', timerEnabled: true, soundEnabled: true }),
    );
    // 3. Clear auth (removes token + user from localStorage)
    dispatch(logout());
  }
  ...
  // Auto-save & achievement sync on changes — scoped to current user
  useEffect(() => {
    ...
    const unsubscribe = store.subscribe(() => {
      ...
      if (currentUserId && state.auth.isAuthenticated) {
        ...
        // Achievement sync guard: if new achievement unlocked, push to backend
        const currentAchievements = state.stats.achievements || [];
        if (currentAchievements.length > lastAchievements.length) {
          const newIds = currentAchievements.filter(
            (id) => !lastAchievements.includes(id),
          );
          lastAchievements = [...currentAchievements]; // update for next check
        ...
        } else if (currentAchievements.length < lastAchievements.length) {
          // Reset case (logout or clear)
        ...
        <header className="header">
          <NavLink to="/" className="header-logo">
            ...
            <span className="header-logo-text">
              <span className="logo-logic">Logic </span>
              <span className="logo-looper">Looper</span>
            </span>
          </NavLink>
