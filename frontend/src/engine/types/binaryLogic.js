      ...
      const gate = {
        ...
        inputs:
          gateType === 'NOT'
            ...
            : [
                prevOutputs[g % prevOutputs.length],
                prevOutputs[(g + 1) % prevOutputs.length],
              ],
        output: null,
      };
...
export function generate(rng, difficulty = 1) {
  ...
  return {
    ...
    questions,
    solution: questions.reduce((acc, q) => {
      acc[q.gateId] = q.answer;
      return acc;
    }, {}),
  };
}
...
export function validate(userAnswers, puzzleData) {
  return puzzleData.questions.every(
    (q) => parseInt(userAnswers[q.gateId]) === q.answer,
  );
}
