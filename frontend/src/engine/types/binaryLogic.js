 ...
 * Enhanced with NAND and NOR gates.
 */

const BASIC_GATES = ['AND', 'OR', 'XOR', 'NOT'];
const ADVANCED_GATES = ['AND', 'OR', 'XOR', 'NOT', 'NAND', 'NOR'];
...
function buildCircuit(rng, depth, useAdvanced) {
    const gates = [];
    const inputs = [];
    const numInputs = depth <= 1 ? 3 : 4;
    const gateTypes = useAdvanced ? ADVANCED_GATES : BASIC_GATES;

    for (let i = 0; i < numInputs; i++) {
        inputs.push({
            id: `in_${i}`,
            value: randInt(0, 1, rng),
    ...
    for (let layer = 0; layer < depth; layer++) {
        ...
        for (let g = 0; g < Math.max(1, prevOutputs.length - 1); g++) {
            const gateType = gateTypes[randInt(0, gateTypes.length - 1, rng)];
            const gate = {
                id: `gate_${layer}_${g}`,
                type: gateType,
                inputs: gateType === 'NOT'
            ...
            gate.output = gateType === 'NOT'
                ? evalGate(gateType, inVals[0])
                : evalGate(gateType, inVals[0], inVals[1]);
...
export function generate(rng, difficulty = 1) {
    ...
    const useAdvanced = difficulty >= 1.2;
    const { inputs, gates } = buildCircuit(rng, depth, useAdvanced);
