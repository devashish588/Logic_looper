...
function Heatmap({ justSolved }) {
  ...
  // Build month labels as a sparse array matching week columns
  const monthCells = useMemo(() => {
    const cells = new Array(weeks.length).fill(null);
    for (const ml of monthLabels) {
      cells[ml.colIdx] = ml.month;
    }
    return cells;
  }, [weeks, monthLabels]);
...
export default function Stats() {
  ...
  // Initialize synchronously from Redux so first render shows correct values
        ...
        try {
          ...
          if (!cancelled && entries.length > 0) {
            const { calculateStreaks } =
              await import('../utils/streakCalculator.js');
