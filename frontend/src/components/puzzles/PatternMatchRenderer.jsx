...
const ShapeSVG = ({ shape, color, rotation }) => {
  ...
  const shapes = {
    ...
    star: (
      <svg className="shape-svg" viewBox="0 0 40 40" style={style}>
        <polygon
          points="20,2 25,14 38,16 28,26 31,38 20,32 9,38 12,26 2,16 15,14"
          fill={color}
        />
      </svg>
    ),
...
export default function PatternMatchRenderer({
  puzzle,
  onAnswer,
  currentRound = 0,
}) {
  ...
  return (
    <div className="puzzle-area">
      ...
      {/* Options */}
      <p className="text-muted mt-24 mb-8" style={{ fontSize: '0.8rem' }}>
      ...
      <div className="pattern-container">
        {round.options.map((item, i) => (
          <motion.div
            key={i}
            className={`pattern-shape pattern-option ${
